...
- Editor boot warms the lazy Audio Mixer panel chunk in the background and preloads, decodes, and retains every wooden skin WebP asset as hidden CSS backgrounds when the skin is enabled, so the first switch into the `AUDIO EDIT` layout does not pay the cold CSS-image load/decode cost. After project or composition load, data-only nested composition audio clips also start a delayed background mixdown warmup that fills their buffer and waveform cache without creating playback audio elements during restore.
- Dock layout transitions run an idle snapshot/measure/WAAPI warmup after the first render and avoid deep-cloning live-animated Timeline/Preview panels, reducing the first cold switch between `VIDEO EDIT` and `AUDIO EDIT`.
