...
- Move only proven active controls out of `TimelineClip`.
  ...
  - Done for read-only audio-region selection/gain visuals through `ClipAudioRegionControls`; interaction commands and legacy-body removal remain pending.
...
- Extract audio, spectral, video-bake, and stem modules.
  ...
  - In progress for the audio-region shell module: visuals are in shell, but move/resize/gain/context-menu commands still need extraction before legacy-body skip.
  - Next recommended parallel slices from the read-only agent pass: audio-region command extraction, video-bake after hit-area parity tests, spectral extraction only with DOM spectrogram retirement deferred until real canvas spectrogram parity.
