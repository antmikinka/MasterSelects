...
- Extract audio, spectral, video-bake, and stem modules.
  ...
  - Next recommended parallel slices from the read-only agent pass: fade duration-positioned handle/curve parity next; audio-region shell module as the next larger parity slice; video-bake after hit-area parity tests; spectral extraction only, with DOM spectrogram retirement deferred until real canvas spectrogram parity.
