...
- Move source waveform generation and thumbnail visible loads out of `TimelineClipCanvas`.
  ...
  - Pending refinement from the thumbnail reload audit: resolve generation from `MediaFile.url` when lazy media hydration has not attached `source.videoElement`, add a small thumbnail-generation concurrency cap, and add URL-based `thumbnailCacheService.generateForSourceUrl()` coverage.
