...
- Current retirement candidates:
  - Full `TimelineClip` mounting inside `.timeline-canvas-dom-overlay` once shell modules cover active controls.
    - Done for trim-only, hover-only, context-menu-only, context-menu-plus-hover, drag/multi-drag-only, drag/multi-drag-plus-hover, stem-only, fade-only, keyframe-only, audio-region-only, and video-bake-only shells; spectral and mixed module shells still need shell parity before deletion.
...
- Extract audio, spectral, video-bake, and stem modules.
  ...
  - Next recommended parallel slice from the read-only agent pass: spectral extraction only with DOM spectrogram retirement deferred until real canvas spectrogram parity.
