...
- Move only proven active controls out of `TimelineClip`.
  ...
  - Done for audio-region selection/gain visuals plus move, resize, gain, fade-in, and fade-out interactions through `ClipAudioRegionControls`; context-menu extraction and legacy-body removal remain pending.
...
- Extract audio, spectral, video-bake, and stem modules.
  ...
  - In progress for the audio-region shell module: visuals and move/resize/gain interactions are in shell, but context-menu commands still need extraction before legacy-body skip.
  - Next recommended parallel slices from the read-only agent pass: audio-region context-menu extraction, video-bake after hit-area parity tests, spectral extraction only with DOM spectrogram retirement deferred until real canvas spectrogram parity.
...
- targeted audio-region shell visual tests: `ClipInteractionShell.contract.test.tsx` and `TimelineTrack.test.tsx`
