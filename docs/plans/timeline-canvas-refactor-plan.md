...
- Current retirement candidates:
  ...
  - Remaining `renderLegacyClip` fallback in non-canvas mode, composition-exit overlays, and new-track drag previews after those paths are either canvas/shell-backed or explicitly kept as compatibility paths.
    - Done: renamed the old generic `TimelineTrackProps.renderClip` contract to `renderLegacyClip` so new canvas paths cannot accidentally depend on a full DOM clip renderer.
...
- Replace generic `renderClip` usage with typed shell props for new paths.
  - Done for the `TimelineTrack` public contract: the only remaining full clip renderer prop is explicitly named `renderLegacyClip`.
...
- Extract audio, spectral, video-bake, and stem modules.
  ...
  - Next recommended parallel slice from the read-only agent pass: remaining `renderLegacyClip` fallback audit and full overlay removal audit, with DOM spectrogram retirement still deferred.
