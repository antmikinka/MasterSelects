...
- Current retirement candidates:
  - Full `TimelineClip` mounting inside `.timeline-canvas-dom-overlay` once shell modules cover active controls.
    - Done for trim-only, context-menu-only, context-menu-plus-hover, drag/multi-drag-only, and drag/multi-drag-plus-hover shells; fade/keyframe/audio/spectral/video-bake/stem/hover-only still need shell parity before deletion.
