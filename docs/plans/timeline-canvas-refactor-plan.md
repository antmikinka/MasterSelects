...
- The former `TimelineTrack` full-renderer prop is gone. The remaining `renderLegacyClip` helper is local to `Timeline.tsx` for composition-exit overlays and new-track drag previews.
...
- Current retirement candidates:
  ...
  - Remaining `renderLegacyClip` fallback in composition-exit overlays and new-track drag previews after those paths are either canvas/shell-backed or explicitly kept as compatibility paths.
...
- Extract audio, spectral, video-bake, and stem modules.
  ...
  - Next recommended parallel slice from the read-only agent pass: remaining local `renderLegacyClip` usage audit and full overlay removal audit, with DOM spectrogram retirement still deferred.
