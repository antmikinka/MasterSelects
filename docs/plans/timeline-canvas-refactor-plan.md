...
- Current retirement candidates:
  - Full `TimelineClip` mounting inside `.timeline-canvas-dom-overlay` once shell modules cover active controls.
    - Done for trim-only, hover-only, context-menu-only, context-menu-plus-hover, drag/multi-drag-only, drag/multi-drag-plus-hover, stem-only, fade-only, keyframe-only, audio-region-only, spectral-region-only, and video-bake-only shells; mixed module shells still need shell parity before deletion.
...
- Extract audio, spectral, video-bake, and stem modules.
  ...
  - Next recommended parallel slice from the read-only agent pass: mixed-module legacy-skip parity and renderClip contract retirement, with DOM spectrogram retirement still deferred.
