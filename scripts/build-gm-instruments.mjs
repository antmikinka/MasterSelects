...
//   node scripts/build-gm-instruments.mjs all drums     # everything
//
...
function buildZone(presetZone, presetGlobal, instZone, instGlobal) {
...
function buildPreset(preset, { isDrum }) {
  const zones = [];
  for (const pz of preset.zones) {
    ...
    for (const iz of inst.zones) {
      ...
      const zone = buildZone(pz, preset.globalZone, iz, inst.globalZone);
...
const wantDrums = args.includes('drums');
const measureOnly = args.includes('measure');
const melodicArg = args.find((a) => a !== 'drums' && a !== 'measure');
...
if (measureOnly) {
  ...
  for (const [preset, isDrum] of presets) {
    ...
    for (const z of asset.zones) {
      zones.push({ samples: Math.round((z.pcm.length * 3 / 4) / 4), rate: z.sampleRate ?? asset.sampleRate, loopEnd: z.loopEnd });
    }
  }
  ...
  recipe('Float32, native rate (current)', project(4, null, null));
  recipe('Int16, native rate', project(2, null, null));
...
let programs = [];
if (melodicArg === 'all') programs = [...melodic.keys()].sort((a, b) => a - b);
...
for (const program of programs) {
  ...
  const asset = buildPreset(preset, { isDrum: false });
...
if (wantDrums) {
  const kit = sf.presets.find((p) => p.header.bank === 128) ?? sf.presets.find((p) => /drum|kit|standard/i.test(p.header.name));
  if (!kit) console.warn('  drums: no bank-128 preset found');
  else {
    const asset = buildPreset(kit, { isDrum: true });
